...
Route::group(["middleware" => ["api"]], function () {
    ...
    Route::group(["prefix" => "admin", "middleware" => ["admin", "language"], "as" => "admin."], function () {
        ...
        // Store Routes
        Route::put("stores/{store_id}/update-status", [\Modules\Core\Http\Controllers\StoreController::class, "updateStatus"])->name("stores.status");
        ...
        // Channels Routes
        Route::put("channels/{channel_id}/update-status", [\Modules\Core\Http\Controllers\ChannelController::class, "updateStatus"])->name("channels.status");
        Route::resource("channels", ChannelController::class)->except(["create", "edit"]);

        // Websites Routes
        Route::put("websites/{website_id}/update-status", [\Modules\Core\Http\Controllers\WebsiteController::class, "updateStatus"])->name("websites.status");
