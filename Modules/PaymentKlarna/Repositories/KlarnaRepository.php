...
class KlarnaRepository extends BasePaymentMethodRepository implements PaymentMethodInterface
{
	...
	public function getPostData(): array
	{
		...
		return [
			...
			"order_amount" => (float) $this->parameter->sub_total, //$this->parameter->grand_total,
			"order_tax_amount" => (float) 220988.654,//$this->parameter->total_tax,//$this->parameter->total_tax,
			"order_lines" => $order->order_items->map(function ($order_item) use ($order) {
				...
				$tax_rate = ($order_item->tax_rate * 100);
				return [
					"type" => "physical",
					...
					"total_tax_amount" => (float) $total_amount - ($total_amount*10000) / (10000 + $tax_rate),//($order_item->tax_amount * $order_item->qty)
				];
			})->toArray(),
			"merchant_urls" => [
